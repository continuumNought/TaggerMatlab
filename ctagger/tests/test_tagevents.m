...
[fMore, hMore, eMore] = eventTags.split(['type;;' values.moreEvents],  false);
assertTrue(isempty(hMore));
dTagsMore = dataTags(hMore);
...
[f4, h4, e4] = eventTags.split('type;; code 1, event 1, /a/b/c, /def', false);
dTagsNew4 = dataTags(h4);
...
eTagsNew5 = eventTags('', e5, 'Field', 'type');
dTags5 = dataTags('');
...
assertTrue(isa(dTagsNew6, 'dataTags'));
